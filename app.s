...
//---------------------------------------- CODIGO ------------------------------------
	...
	mov x1, #60						// coordenada x del vertice inferior izquierdo
	mov x2, #60	
	movz x3, 0xFF, lsl 16       	// defino color principal (BLANCO)
	movk x3, 0xFFFF, lsl 0      	// termino de definir color ppal
	mov x4, #100					// Alto
	mov x5, #200					// Ancho
	mov x9, #500
	loop_animacion:
	bl backup_rectangulo			//Backup de pixels a modificar
    bl adorno_luminoso	        	//Pintamos la figura a animar
	movz x10, 0xFF, lsl 16       	// defino color principal (ROSA)
	movk x10, 0x0FFF, lsl 0      	// termino de definir color ppal
	bl delay
...
//Pinta un pixel 1x1 en determinada posicion
	pintar_punto:
		...
		stur w3,[x0]
		br lr
// -----------------pintar_rectangulo_aux(x, y, color): -----------------
...
//Pinta una circunferencia de radio r y centro (xc,yx)
	pintar_circunferencia:
		...
		// Centro x1 = cx, x2 = cy, x3 = color
			...
			br lr //retorno
//-----------------------------------------------------------------------------------
// -----------------pintar_circulo (xc, yc, radio, color): -----------------
